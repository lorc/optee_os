...
void virt_init_memory(struct tee_mmap_region *memory_map)
{
	...
	if (!tee_mm_init(&virt_mapper_pool, CFG_TEE_RAM_START,
			 CFG_TA_RAM_START + CFG_WHOLE_TEE_SECURE_RAM_SIZE,
			 SMALL_PAGE_SHIFT,
			 TEE_MM_POOL_KMALLOC))
		panic("Can't create pool with free pages");
	DMSG("Created virtual mapper pool from %x to %x\n",
	     CFG_TEE_RAM_START, CFG_TA_RAM_START + CFG_WHOLE_TEE_SECURE_RAM_SIZE);
...
static int configure_guest_ctx_mem(struct guest_context *ctx)
{
	...
	ctx->ta_ram = tee_mm_alloc(&virt_mapper_pool, CFG_TA_RAM_SIZE);
	if (!ctx->ta_ram) {
